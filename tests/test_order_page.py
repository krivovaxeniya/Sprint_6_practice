...
class TestMakeOrder:
    ...
    @pytest.mark.parametrize("name, surname, adress, phonenumber, subway_locator, date_locator, srok, color, comment_text",
                             Data.users)
    def test_go_to_order_page_and_make_order(self, driver, locator_button, name, surname, adress, phonenumber, subway_locator, date_locator, srok, color, comment_text):
        main_page = MainPageSamokat(driver)
        main_page.click_on_cookie_button()
        main_page.scroll_to_element_and_click(locator_button)
        ...
        assert 'Заказ оформлен' in order_page.get_text_from_success_order_form()
